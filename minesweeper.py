    ...
    def add_knowledge(self, cell, count):
        ...
        for row in range(-1, 2, 1):
            for col in range(-1, 2, 1):
                if (cell_row + row) < self.height and (cell_row + row) > -1 and (cell_col + col) < self.width and (cell_col + col) > -1 and (cell_row + row, cell_col + col) != cell:
                    if (cell_row + row, cell_col + col) in self.mines:
                        count -= 1
                    elif (cell_row + row, cell_col + col) not in self.safes:
                        neighbours.add((cell_row + row, cell_col + col))
        ...
        for pos1 in range(knowledge):
            for pos2 in range(pos1 + 1, knowledge):
                sentence1 = self.knowledge[pos1]
                sentence2 = self.knowledge[pos2]
